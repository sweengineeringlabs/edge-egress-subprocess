//! Allow-list basename normalisation and membership check.
//!
//! Normalisation rules:
...
/// Command allow-list checker with basename normalisation.
pub(crate) struct List;

impl List {
    /// Normalise a command path to the basename used for allow-list comparison.
    ...
    ///
    /// Both sides are normalised before comparison so callers can list `echo` and
...
mod tests {
    ...
    fn test_normalise_strips_directory_prefix() {
        assert_eq!(List::normalise("/usr/bin/echo"), "echo");
    }

    #[test]
    fn test_normalise_absolute_windows_path_strips_prefix() {
        let result = List::normalise("C:\\Windows\\System32\\cmd.exe");
    ...
    fn test_is_allowed_multiple_entries_matches_any() {
        ...
        assert!(List::is_allowed("cat", &allowed));
    }
}
